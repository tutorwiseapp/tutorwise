...
test.describe('Account > Professional Info', () => {
  ...
  test('should submit form successfully', async ({ page }) => {
    ...
    // Now should be enabled
    await expect(saveButton).toBeEnabled();
    await page.waitForFunction(selector => !document.querySelector(selector).disabled, 'button[name="Save Template"]');

    // Submit form
    await saveButton.click();

    // Should show success toast (wait for it to appear)
    await expect(page.getByRole('status', { name: /Template saved/ })).toBeVisible({ timeout: 10000 });
    await expect(page.getByRole('status', { name: /Changes won't affect your existing listings/ })).toBeVisible({ timeout: 10000 });
  });
