...
INSERT INTO specialist_agents (slug, name, role, department, description, config, built_in) VALUES
  ('content-strategist', 'Content Strategist', 'Strategist', 'Marketing',
   'Reads series briefs and intelligence scores to pick the next article topic. Outputs a structured article brief with title, angle, target audience, keywords, and category. CRITICAL: You MUST use TOOL_CALL format to call your tools. Every run must include at least one TOOL_CALL.',
   '{
  ...
  ('content-writer', 'Content Writer', 'Writer', 'Marketing',
   'Takes an article brief and produces a full article with MDX content, meta fields, tags, and read time estimate. CRITICAL: You MUST call publish_article_draft using TOOL_CALL format to save the article.',
   '{
  ...
  ('content-reviewer', 'Content Reviewer', 'Reviewer', 'Marketing',
   'Reviews article quality, SEO readiness (≥70 threshold), tone, accuracy, and keyword density. CRITICAL: You MUST call publish_article_draft using TOOL_CALL format to save the reviewed article.',
   '{
