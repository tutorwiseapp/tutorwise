-- Migration: Add job listing fields to listings table
-- Version: 193
-- Date: 2026-01-20
-- Purpose: Enable agents to create comprehensive job listings for recruiting tutors
-- Related: JobListingForm component with 30 fields across 7 sections

-- Add job-specific columns to listings table
ALTER TABLE listings
  -- Section 1: Job Basics (7 fields)
  ADD COLUMN employment_type VARCHAR(50),  -- 'full-time', 'part-time', 'contract', 'freelance'
  ADD COLUMN contract_length VARCHAR(50),  -- 'permanent', 'fixed-3m', 'fixed-6m', 'fixed-1y', 'temporary'
  ADD COLUMN start_date DATE,
  ADD COLUMN end_date DATE,
  ADD COLUMN application_deadline DATE,

  -- Section 2: Teaching Details (2 new fields - subjects & levels already exist)
  ADD COLUMN student_numbers VARCHAR(50),  -- '1-5', '6-10', '11-20', '20+'
  ADD COLUMN class_type TEXT[] DEFAULT '{}',  -- ['one-to-one', 'small-group', 'medium-group', 'large-group']

  -- Section 3: Location & Schedule (5 fields)
  ADD COLUMN delivery_mode TEXT[] DEFAULT '{}',  -- ['online', 'in-person', 'hybrid']
  ADD COLUMN work_location TEXT,  -- Full location string (e.g., "London, SW1")
  ADD COLUMN hours_per_week VARCHAR(50),  -- 'under-10', '10-20', '20-30', '30-40', '40+'
  ADD COLUMN schedule_flexibility VARCHAR(50),  -- 'fixed', 'flexible', 'weekends', 'evenings'
  ADD COLUMN timezone_requirements TEXT,  -- Free-form text (e.g., "Must work UK hours (9am-5pm GMT)")

  -- Section 4: Compensation & Benefits (5 fields)
  ADD COLUMN compensation_type VARCHAR(50),  -- 'hourly', 'annual', 'per-session', 'commission'
  ADD COLUMN compensation_min NUMERIC(10, 2),
  ADD COLUMN compensation_max NUMERIC(10, 2),
  ADD COLUMN benefits TEXT[] DEFAULT '{}',  -- ['flexible-schedule', 'professional-dev', 'paid-training', etc.]
  ADD COLUMN additional_benefits TEXT,

  -- Section 5: Requirements & Qualifications (5 fields)
  ADD COLUMN minimum_qualifications TEXT[] DEFAULT '{}',  -- ['university-degree', 'masters', 'phd', etc.]
  ADD COLUMN teaching_credentials TEXT[] DEFAULT '{}',  -- ['qtls-qts', 'pgce', 'teaching-license', 'none']
  ADD COLUMN minimum_experience VARCHAR(50),  -- 'entry', 'junior', 'mid', 'senior', 'any'
  ADD COLUMN dbs_check VARCHAR(50),  -- 'required', 'assist', 'no'
  ADD COLUMN other_requirements TEXT,

  -- Section 6: Application Process (2 fields)
  ADD COLUMN how_to_apply VARCHAR(50),  -- 'tutorwise', 'network', 'organisation'
  ADD COLUMN application_instructions TEXT,

  -- Section 7: About Organisation (2 fields)
  ADD COLUMN about_organisation TEXT,
  ADD COLUMN organisation_type VARCHAR(50);  -- 'tutoring-agency', 'company', 'school', 'college', 'university', 'charity', 'other'
...
-- Add indexes for job listing queries (performance optimization)
CREATE INDEX idx_listings_employment_type ON listings(employment_type);
CREATE INDEX idx_listings_delivery_mode ON listings USING GIN(delivery_mode);
...
CREATE INDEX idx_listings_minimum_qualifications ON listings USING GIN(minimum_qualifications);
CREATE INDEX idx_listings_teaching_credentials ON listings USING GIN(teaching_credentials);
...
COMMENT ON COLUMN listings.delivery_mode IS 'How tutoring is delivered: online, in-person, or hybrid';
