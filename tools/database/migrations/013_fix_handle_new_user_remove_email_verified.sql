-- Migration 013: Fix handle_new_user - remove email_verified column reference
-- Purpose: The email_verified column doesn't exist in profiles table, causing signup failures
-- This migration removes the reference to email_verified
...
BEGIN
  ...
  -- Insert profile with all available data (removed email_verified as it doesn't exist)
...
COMMENT ON FUNCTION handle_new_user() IS 'Automatically creates a profile when a user signs up. Generates referral_id, extracts name data from auth metadata (from email/password signup or OAuth providers like Google), and parses full_name into first_name/last_name if needed.';
