-- Migration 160: Add Task Comments and File Attachments
-- Created: 2026-01-04
-- Description: Adds collaboration features to org tasks - comments and file attachments
...
CREATE INDEX idx_task_comments_task ON org_task_comments(task_id);
CREATE INDEX idx_task_comments_user ON org_task_comments(user_id);
CREATE INDEX idx_task_comments_created ON org_task_comments(created_at DESC);

-- ============================================================================
-- 2. CREATE TASK ATTACHMENTS TABLE
-- ============================================================================

CREATE TABLE IF NOT EXISTS org_task_attachments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  task_id UUID NOT NULL REFERENCES org_tasks(id) ON DELETE CASCADE,
  ...
  CONSTRAINT check_file_name_not_empty CHECK (trim(file_name) <> ''),
  CONSTRAINT check_storage_path_not_empty CHECK (trim(storage_path) <> '')
);

-- Indexes for attachments
CREATE INDEX idx_task_attachments_task ON org_task_attachments(task_id);
CREATE INDEX idx_task_attachments_uploader ON org_task_attachments(uploaded_by);
CREATE INDEX idx_task_attachments_uploaded ON org_task_attachments(uploaded_at DESC);
