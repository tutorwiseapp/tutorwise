...
class SimpleGitHubSync {
    ...
    async syncProjectData() {
        try {
            ...
            if (project)
                console.log('   - .ai/github/project-board.md');
        }
        catch (error) {
    ...
    async getRecentPullRequests(limit = 15) {
    ...
    formatRepositoryOverview(repo, issues, pullRequests) {
...
## Priority Issues
${openIssues.slice(0, 5).map(issue => `- **#${issue.number}**: ${issue.title} ${issue.labels.length ? `[${issue.labels.join(', ')}]` : ''}`).join('\n') || '- No open issues'}

## Active Pull Requests
${openPRs.slice(0, 5).map(pr => `- **#${pr.number}**: ${pr.title} by ${pr.user} ${pr.labels.length ? `[${pr.labels.join(', ')}]` : ''}`).join('\n') || '- No open pull requests'}
...
`;
    ...
    formatPullRequestAsMarkdown(pr) {
...
## Board Overview
${Object.entries(statusGroups).map(([status, items]) => `### ${status} (${items.length})
${items.map(item => `- **#${item.content.number}**: ${item.content.title} ${item.content.assignees.length ? `(${item.content.assignees.join(', ')})` : ''}`).join('\n')}`).join('\n\n')}

---
*Last synced*: ${new Date().toISOString()}
`;
    }
    async updateMainContext({ repo, issues, pullRequests, project }) {
        const openIssues = issues.filter((i) => i.state === 'open' && !i.pull_request);
        const openPRs = pullRequests.filter((pr) => pr.state === 'open');
...
**Priority Issues**:
${openIssues.slice(0, 3).map((issue) => `- **#${issue.number}**: ${issue.title} ${issue.labels.length ? `[${issue.labels.join(', ')}]` : ''}`).join('\n') || '- No open issues'}

**Active Pull Requests**:
${openPRs.slice(0, 3).map((pr) => `- **#${pr.number}**: ${pr.title} by ${pr.user}`).join('\n') || '- No open pull requests'}
