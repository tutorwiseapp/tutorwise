-- Migration 133: Add booking_source to distinguish listing vs profile bookings
...
CREATE TYPE booking_source_enum AS ENUM ('listing', 'profile');

COMMENT ON TYPE booking_source_enum IS
  'Source of the booking: listing (via service listing page) or profile (direct from tutor profile page)';

-- ============================================================
-- 2. ADD BOOKING_SOURCE FIELD
-- ============================================================

-- Add booking_source column
ALTER TABLE public.bookings
ADD COLUMN booking_source booking_source_enum;
...
SET booking_source = CASE
  WHEN listing_id IS NULL THEN 'profile'::booking_source_enum
  ELSE 'listing'::booking_source_enum
END;
...
CREATE INDEX idx_bookings_booking_source
  ON public.bookings(booking_source);

COMMENT ON INDEX idx_bookings_booking_source IS
...
VALUES (
  'schema.add_booking_source',
  'Bookings',
  jsonb_build_object(
    'migration', '133',
    'changes', ARRAY[
      'Created booking_source_enum (listing, profile)',
      'Added booking_source column to bookings table',
