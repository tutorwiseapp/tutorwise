-- Migration 048: Update payment webhook to trigger review sessions
-- Purpose: Modify handle_successful_payment to set booking status='Completed'
-- Author: Senior Architect + Claude AI
-- Date: 2025-11-08
-- Related: reviews-solution-design-v4.5.md, migration 030, migration 045

-- =====================================================================
-- This migration updates the handle_successful_payment RPC function
-- to set the booking status to 'Completed' when payment is confirmed,
-- which will automatically trigger the creation of a review session
-- via the trigger created in migration 045.
-- =====================================================================
...
BEGIN
  ...
  -- =================================================================
  -- STEP 6: Update the booking table (UPDATED FOR REVIEWS v4.5)
  -- =================================================================
  -- Set both payment_status='Confirmed' AND status='Completed'
  -- This triggers the review session creation (migration 045)
...
COMMENT ON FUNCTION public.handle_successful_payment IS
  'Updated for v4.5 reviews: Sets booking status to Completed, triggering review session creation';

-- Audit log
INSERT INTO public.audit_log (action_type, module, details)
VALUES (
  'review_system.payment_webhook_updated',
  'Reviews',
  jsonb_build_object(
    'migration', '048',
    'function', 'handle_successful_payment',
    'changes', 'Sets booking status=Completed to trigger review sessions',
    'timestamp', NOW()
  )
);
