-- Migration 024: Remove default 'seeker' role assignment
-- Purpose: Let users choose their role during onboarding instead of auto-assigning 'seeker'
-- Date: 2025-10-21

-- Update the trigger to NOT set any default role
...
BEGIN
  BEGIN
    ...
    -- Insert profile WITHOUT any default role
    -- User will choose their role during onboarding
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    -- Fallback: try minimal profile WITHOUT default role
    BEGIN
      INSERT INTO public.profiles (id, referral_id, email, roles, active_role)
      VALUES (NEW.id, v_referral_id, NEW.email, ARRAY[]::text[], NULL);
    EXCEPTION WHEN OTHERS THEN
...
COMMENT ON FUNCTION handle_new_user() IS 'Creates profile on signup WITHOUT default role. Users choose their role during onboarding. Uses first_name/last_name as standard, supports given_name/family_name for OAuth. Has robust error handling.';
