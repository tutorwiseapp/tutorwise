/**
 * Filename: AdminAITutorDetailModal.tsx
...
import styles from './AdminAITutorDetailModal.module.css';
...
interface AdminAITutorDetailModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function AdminAITutorDetailModal({
  isOpen,
  onClose,
  aiTutor,
  onAITutorUpdated,
}: AdminAITutorDetailModalProps) {
  if (!aiTutor) return null;

  const handleViewAITutor = () => {
    window.open(`/ai-tutors/${aiTutor.id}`, '_blank');
  };

  const handleEditAITutor = () => {
    window.open(`/ai-tutors/${aiTutor.id}/edit`, '_blank');
  };
