/**
 * Filename: apps/web/src/app/api/blog/saves/route.ts
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[API] Error in POST /api/blog/saves:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

/**
 * DELETE /api/blog/saves
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('[API] Error in DELETE /api/blog/saves:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
