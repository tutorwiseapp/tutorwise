/**
 * Filename: api/ai-tutors/bundles/my-bundles/route.ts
...
/**
 * GET /api/ai-tutors/bundles/my-bundles
 * Get client's active bundle purchases
 *
 * Query params:
 * - ai_tutor_id: filter by specific AI tutor
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let query = supabase
      ...
      .select(`
        *,
        ...
        ai_tutor:ai_tutors!ai_tutor_id(
          id,
          name,
          avatar_url
        )
    ...
    if (aiTutorId) {
      query = query.eq('ai_tutor_id', aiTutorId);
    }
