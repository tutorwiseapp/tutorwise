 ...
 *
 * @module components/feature/ai-tutors/session/useAITutorChat
 */
...
export function useAITutorChat(options: UseAITutorChatOptions): UseAITutorChatReturn {
  ...
  const loadSession = async () => {
    try {
      ...
      const response = await fetch(`/api/ai-tutors/sessions/${sessionId}`);
  ...
  const sendMessage = useCallback(async (messageText: string) => {
    ...
    try {
      const response = await fetch(`/api/ai-tutors/sessions/${sessionId}/messages`, {
        method: 'POST',
  ...
  const endSession = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/ai-tutors/sessions/${sessionId}/end`, {
        method: 'POST',
      });
  ...
  const escalateToHuman = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/ai-tutors/sessions/${sessionId}/escalate`, {
        method: 'POST',
      });
