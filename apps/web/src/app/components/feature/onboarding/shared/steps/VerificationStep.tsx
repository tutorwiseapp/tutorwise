...
/**
 * Shared Verification Step Component
 *
 * A unified component for tutor, agent, and client verification documents.
 * All roles share the same verification fields; only the role context differs.
 */
...
const VerificationStep: React.FC<VerificationStepProps> = ({
  ...
  useEffect(() => {
    if (!isRestored && user?.id) {
      getOnboardingProgress(role)
        .then(savedProgress => {
          const savedData = savedProgress?.progress?.[role]?.verification;

          if (savedData) {
            console.log(`[VerificationStep:${role}] ✅ Restored saved progress:`, savedData);
        ...
        .catch(error => {
          console.error(`[VerificationStep:${role}] Error loading saved progress:`, error);
          setIsRestored(true);
        });
    }
  ...
  const handleBlur = useCallback((fieldName: string) => {
    ...
    setTimeout(() => {
      ...
      console.log(`[VerificationStep:${role}] onBlur save for: ${fieldName}`);
      setIsSaving(true);
      ...
      saveOnboardingProgress({
        ...
        .then(() => console.log(`[VerificationStep:${role}] ✓ Blur save completed`))
        .catch((error) => console.error(`[VerificationStep:${role}] ❌ Blur save failed:`, error))
        .finally(() => {
          setIsSaving(false);
          setEditingField(null);
        });
    }, 150);
  }, [editingField, isSaving, user?.id, combinedData, role]);
  ...
  const handleSelectChange = useCallback((field: string, value: string) => {
    ...
    console.log(`[VerificationStep:${role}] Immediate save for select: ${field}`);
    ...
    saveOnboardingProgress({
      ...
      .then(() => console.log(`[VerificationStep:${role}] ✓ Select save completed`))
      .catch((error) => console.error(`[VerificationStep:${role}] ❌ Select save failed:`, error));
  }, [formData, uploadedFiles, user?.id, role]);

  const handleDateChange = useCallback((field: string, dateStr: string) => {
    const newFormData = { ...formData, [field]: dateStr };
    setFormData(newFormData);

    if (!user?.id) return;

    console.log(`[VerificationStep:${role}] Immediate save for date: ${field}`);
    const newCombinedData = { ...newFormData, ...uploadedFiles };
    saveOnboardingProgress({
      userId: user.id,
      progress: {
      ...
      .then(() => console.log(`[VerificationStep:${role}] ✓ Date save completed`))
      .catch((error) => console.error(`[VerificationStep:${role}] ❌ Date save failed:`, error));
  }, [formData, uploadedFiles, user?.id, role]);
  ...
  const handleNext = () => {
    console.log(`[VerificationStep:${role}] handleNext called`);
    ...
    console.log(`[VerificationStep:${role}] Calling onNext...`);
    // Page handles all database operations
    onNext(dataToSave);
    console.log(`[VerificationStep:${role}] onNext called successfully`);
  };
    ...
    <div className={styles.stepContent}>
      <div className={styles.stepHeader}>
        ...
        <p className={styles.stepSubtitle}>
          {roleDisplayName} Onboarding • Upload your verification documents
        </p>
      </div>
