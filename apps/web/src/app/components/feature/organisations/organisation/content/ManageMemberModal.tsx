...
export default function ManageMemberModal({
  ...
  const [isAdmin, setIsAdmin] = useState<boolean>(member.member_role === 'admin');
  const [isSaving, setIsSaving] = useState(false);

  // Check if admin role feature is available (migration 128 has been run)
  const isAdminFeatureAvailable = member.member_role !== undefined;
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setIsAdmin(member.member_role === 'admin');
    }
  }, [isOpen, member]);
  ...
  const handleAdminToggle = async (checked: boolean) => {
    ...
    setIsSaving(true);
    try {
      await updateMutation.mutateAsync({ role: checked ? 'admin' : 'member' });
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.content}>
          ...
          {/* Admin Role Toggle - only show if migration 128 has been run */}
          {isAdminFeatureAvailable && (
            <div className={styles.field}>
              <label className={styles.checkboxLabel}>
                ...
                <span className={styles.hint}> • Admins can view full organisation analytics and manage members</span>
              </label>
            </div>
          )}
