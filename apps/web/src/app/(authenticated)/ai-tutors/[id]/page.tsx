/**
 * Filename: apps/web/src/app/(authenticated)/ai-tutors/[id]/page.tsx
...
import OverviewTab from '@/app/components/feature/ai-tutors/detail/OverviewTab';
import MaterialsTab from '@/app/components/feature/ai-tutors/detail/MaterialsTab';
import LinksTab from '@/app/components/feature/ai-tutors/detail/LinksTab';
import SessionsTab from '@/app/components/feature/ai-tutors/detail/SessionsTab';
import AnalyticsTab from '@/app/components/feature/ai-tutors/detail/AnalyticsTab';
import SettingsTab from '@/app/components/feature/ai-tutors/detail/SettingsTab';
...
export default function AITutorDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(`/api/ai-tutors/${id}`);
  ...
  const { data: subscription } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/ai-tutors/${id}/subscription`);
  ...
  const handleTabChange = (tabId: string) => {
    ...
    router.push(`/ai-tutors/${id}?${params.toString()}`, { scroll: false });
  };
    ...
    return (
      ...
      >
        <HubEmptyState
          ...
          onAction={() => router.push('/ai-tutors')}
        />
      </HubPageLayout>
    );
  }
