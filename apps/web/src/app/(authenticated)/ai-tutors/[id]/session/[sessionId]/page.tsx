 ...
 *
 * @module app/(authenticated)/ai-tutors/[id]/session/[sessionId]/page
 */

import { use, useState, useRef, useEffect, useCallback } from 'react';
import { useRouter } from 'next/navigation';
import { useAITutorChat } from '@/app/components/feature/ai-tutors/session/useAITutorChat';
import LexiMarkdown from '@/components/feature/lexi/LexiMarkdown';
import SessionTimer from '@/app/components/feature/ai-tutors/session/SessionTimer';
import ReviewModal from '@/app/components/feature/ai-tutors/session/ReviewModal';
...
export default function AITutorSessionPage({ params }: PageProps) {
  ...
  const handleReviewSubmit = useCallback(async (rating: number, reviewText: string) => {
    try {
      await fetch(`/api/ai-tutors/${aiTutorId}/reviews`, {
        method: 'POST',
      ...
      router.push(`/ai-tutors/${aiTutorId}`);
    } catch (err) {
      ...
      {!isSessionActive && (
        <div className={styles.sessionEnded}>
          ...
          <button
            ...
            onClick={() => router.push(`/ai-tutors/${aiTutorId}`)}
          >
            Back to AI Tutor
          </button>
        </div>
