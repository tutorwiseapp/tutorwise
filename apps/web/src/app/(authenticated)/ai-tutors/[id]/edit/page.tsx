/**
 * Filename: apps/web/src/app/(authenticated)/ai-tutors/[id]/edit/page.tsx
 * Purpose: AI Tutor Editor - edit existing AI tutor
 * Route: /ai-tutors/[id]/edit
 * Created: 2026-02-24
...
export default function EditAITutorPage({ params }: EditAITutorPageProps) {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`/api/ai-tutors/${tutorId}`);
  ...
  const handleSubmit = async (data: AITutorFormData, shouldPublish: boolean) => {
    ...
    try {
      // Update AI tutor
      const response = await fetch(`/api/ai-tutors/${tutorId}`, {
        method: 'PATCH',
      ...
      if (shouldPublish && aiTutor?.status !== 'published') {
        ...
        if (aiTutor?.subscription_status !== 'active') {
          try {
            const subResponse = await fetch(`/api/ai-tutors/${tutorId}/subscription`, {
              method: 'POST',
            });
          ...
          try {
            const publishResponse = await fetch(`/api/ai-tutors/${tutorId}/publish`, {
              method: 'POST',
            });
    ...
    return (
      ...
      >
        <div style={{ padding: '2rem', textAlign: 'center' }}>
          ...
          <button onClick={() => router.push('/ai-tutors')}>Back to List</button>
        </div>
      </HubPageLayout>
    );
  }
