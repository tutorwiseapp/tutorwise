...
export default function AgentProfessionalDetailsPage() {
  ...
  useEffect(() => {
    ...
    if (!isLoading && !user) {
      router.push('/login?redirect=/onboarding/agent/professional-details');
      return;
    }
  ...
  const handleNext = async (data: ProfessionalDetailsData) => {
    console.log('[AgentProfessionalDetails] handleNext called', data);
    ...
    try {
      ...
      console.log('[AgentProfessionalDetails] ✓ Saved to role_details');
      ...
      console.log('[AgentProfessionalDetails] ✓ Step marked as completed');

      // Navigate to next step
      router.push('/onboarding/agent/verification');
    } catch (error) {
      console.error('[AgentProfessionalDetails] Error:', error);
