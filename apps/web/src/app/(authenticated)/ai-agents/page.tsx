...
import AIAgentStatsWidget from '@/components/feature/ai-agents/AIAgentStatsWidget';
import AIAgentLimitsWidget from '@/components/feature/ai-agents/AIAgentLimitsWidget';
import AIAgentHelpWidget from '@/components/feature/ai-agents/AIAgentHelpWidget';
import AIAgentTipsWidget from '@/components/feature/ai-agents/AIAgentTipsWidget';
...
type AgentTypeFilter = AgentType | 'all';
...
export default function AIAgentsPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async (id) => {
      ...
      await queryClient.cancelQueries({ queryKey: ['ai-agents', profile?.id] });

      // Snapshot current value for rollback
      const previousTutors = queryClient.getQueryData(['ai-agents', profile?.id]);

      // Optimistically update cache
      queryClient.setQueryData(['ai-agents', profile?.id], (old: AITutor[] = []) =>
  ...
  const archiveMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/ai-agents/${id}/archive`, { method: 'POST' });
  ...
  const filteredAgents = useMemo(() => {
    ...
    if (agentTypeFilter !== 'all') {
      result = result.filter(t => (t.agent_type || 'tutor') === agentTypeFilter);
    }
    if (searchQuery.trim()) {
  ...
  const handleFilterChange = (newFilter: FilterType) => {
    ...
    router.push(`/ai-agents${params.toString() ? `?${params.toString()}` : ''}`, { scroll: false });
  };
  ...
  const handlePublish = async (id: string) => {
    ...
    if (tutor.subscription_status !== 'active') {
      ...
      try {
        const res = await fetch(`/api/ai-agents/${id}/subscription`, { method: 'POST' });
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `ai-agents-${new Date().toISOString().split('T')[0]}.csv`;
          ...
          filters={
            <div className={filterStyles.filtersContainer}>
              ...
              <UnifiedSelect
                ...
                onChange={(value) => setAgentTypeFilter(String(value) as AgentTypeFilter)}
                options={[
                  { value: 'all', label: 'All Types' },
                  { value: 'tutor', label: 'Tutor' },
                  { value: 'coursework', label: 'Coursework' },
                  { value: 'study_buddy', label: 'Study Buddy' },
                  { value: 'research_assistant', label: 'Research' },
