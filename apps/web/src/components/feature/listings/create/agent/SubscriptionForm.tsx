/**
 * Filename: SubscriptionForm.tsx
 * Purpose: Subscription tutoring service listing creation form
 * Pattern: Copied from GroupSessionForm.tsx and customized for subscriptions
 * Created: 2026-02-24
...
import CustomTimePicker from '@/components/feature/listings/create/shared-components/CustomTimePicker';
import DatePicker from '@/components/ui/forms/DatePicker';
import Button from '@/components/ui/actions/Button';
import HubListItem from '@/components/hub/content/HubListItem/HubListItem';
import { useFormConfigs } from '@/hooks/useFormConfig';
import { formatMultiSelectLabel } from '@/app/utils/formHelpers';
import toast from 'react-hot-toast';
import styles from './GroupSessionForm.module.css'; // Reusing styles
...
type EditingField = 'title' | 'subjects' | 'levels' | 'description' | 'session_duration' |
  'subscription_monthly_rate' | 'delivery_mode' | 'images' | 'frequency' | 'sessions_per_week' | null;
...
export default function SubscriptionForm({ onSubmit, onCancel, isSaving = false, initialData = {} }: SubscriptionFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    session_duration: '',
    subscription_monthly_rate: (initialData as any).subscription_monthly_rate?.toString() || '',
    delivery_mode: '',
  ...
  useEffect(() => {
    ...
    localStorage.setItem('subscription_draft', JSON.stringify(draftData));
  ...
  // Calculate session limit per period based on frequency
  const calculateSessionLimit = () => {
    const sessionsPerWeek = parseInt(formData.sessions_per_week) || 5;
    if (formData.frequency === 'daily') {
      return sessionsPerWeek * 4; // ~4 weeks per month
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.subscription_monthly_rate || parseFloat(formData.subscription_monthly_rate) <= 0) {
      errors.push('Please enter a valid monthly subscription rate');
    }
    if (!formData.frequency) {
      errors.push('Please select a session frequency');
    }
    if (formData.frequency === 'daily' && (!formData.sessions_per_week || parseInt(formData.sessions_per_week) <= 0)) {
      errors.push('Please enter valid sessions per week for daily frequency');
    }
    ...
    try {
      const listingData: any = {
        ...
        subscription_config: {
          ...
          term_time_only: formData.term_time_only,
          session_duration_minutes: parseInt(formData.session_duration),
          sessions_per_week: formData.frequency === 'daily' ? parseInt(formData.sessions_per_week) : undefined,
          session_limit_per_period: calculateSessionLimit(),
          price_per_month_pence: Math.round(parseFloat(formData.subscription_monthly_rate) * 100),
          auto_renew: true
        }
      };
    ...
    <HubForm.Root>
      ...
      <HubForm.Section>
        <HubForm.Grid columns={1}>
          {renderField('title', 'title', 'Subscription Title', 'text', 'E.g., Monthly GCSE Maths Subscription - Daily Tutoring Support', undefined, true)}
        </HubForm.Grid>

        <HubForm.Grid columns={1}>
          {renderField('description', 'description', 'Service Description', 'textarea', 'Describe your subscription offering, what clients can expect, your teaching approach...', undefined, true)}
        </HubForm.Grid>
        ...
        <HubForm.Grid>
          {renderField('frequency', 'frequency', 'Session Frequency', 'select', 'Select frequency', frequencyOptions, true)}
          {formData.frequency === 'daily' && renderField('sessions_per_week', 'sessions_per_week', 'Sessions Per Week', 'number', '5', undefined, true)}
        </HubForm.Grid>

        <HubForm.Grid>
          {renderField('subscription_monthly_rate', 'subscription_monthly_rate', 'Monthly Subscription Rate (£)', 'number', '£100', undefined, true)}
        </HubForm.Grid>

        {/* Term time only checkbox */}
        <HubForm.Grid columns={1}>
          <HubForm.Field label="Term Time Only">
            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
              <input
                type="checkbox"
                name="term_time_only"
                checked={formData.term_time_only}
                onChange={handleChange}
                disabled={isSaving || localIsSaving}
              />
              <span>Exclude school holidays (term time only)</span>
            </label>
          </HubForm.Field>
        </HubForm.Grid>
        ...
        <div style={{ marginTop: '1rem', padding: '1rem', background: '#f3f4f6', borderRadius: '8px' }}>
          <h4 style={{ margin: '0 0 0.5rem 0', fontSize: '0.875rem', fontWeight: 600 }}>Subscription Preview</h4>
          <p style={{ margin: 0, fontSize: '0.875rem', color: '#6b7280' }}>
            Clients will pay <strong>£{formData.subscription_monthly_rate || '0'}/month</strong> for{' '}
            {formData.frequency === 'daily' && `${formData.sessions_per_week || '0'} sessions per week (~${calculateSessionLimit()} per month)`}
            {formData.frequency === 'weekly' && '1 session per week (~4 per month)'}
            {formData.frequency === 'fortnightly' && '1 session every 2 weeks (~2 per month)'}
            {formData.term_time_only && ', term time only'}
          </p>
        </div>
      </HubForm.Section>
      ...
      <HubForm.Actions>
        ...
        <Button
          ...
          disabled={isSaving || localIsSaving}
        >
          {(isSaving || localIsSaving) ? 'Publishing...' : 'Publish Subscription'}
        </Button>
      </HubForm.Actions>
    </HubForm.Root>
  );
