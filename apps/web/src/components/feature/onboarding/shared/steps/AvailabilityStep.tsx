...
/**
 * Shared Availability Step Component
 *
 * A unified component for tutor, agent, and client availability setup.
 * All roles share the same availability UI; only the role context differs.
 */
...
const AvailabilityStep: React.FC<AvailabilityStepProps> = ({
  ...
  const handleImmediateSave = useCallback(() => {
    ...
    console.log(`[AvailabilityStep:${role}] Immediate save triggered`);
    saveOnboardingProgress({
      ...
      .then(() => console.log(`[AvailabilityStep:${role}] ✓ Immediate save completed`))
      .catch((error) => console.error(`[AvailabilityStep:${role}] ❌ Immediate save failed:`, error));
  }, [user?.id, availabilityData, role]);

  // Restore saved onboarding progress on mount
  useEffect(() => {
    console.log(`[AvailabilityStep:${role}] Restoration useEffect triggered`, {
      isRestored,
      hasUserId: !!user?.id,
      userId: user?.id
    });
    ...
    if (!isRestored && user?.id) {
      console.log(`[AvailabilityStep:${role}] Starting getOnboardingProgress call...`);

      getOnboardingProgress(role)
        .then(savedProgress => {
          console.log(`[AvailabilityStep:${role}] getOnboardingProgress returned:`, savedProgress);

          const savedData = savedProgress?.progress?.[role]?.availability;
          console.log(`[AvailabilityStep:${role}] Extracted availability data:`, savedData);

          if (savedData) {
            console.log(`[AvailabilityStep:${role}] ✅ Restoring saved progress`);

            // Restore general availability
            if (savedData.generalDays) {
              console.log(`[AvailabilityStep:${role}] Setting generalDays:`, savedData.generalDays);
              setGeneralDays(savedData.generalDays);
            } else {
              console.log(`[AvailabilityStep:${role}] No generalDays to restore`);
            }

            if (savedData.generalTimes) {
              console.log(`[AvailabilityStep:${role}] Setting generalTimes:`, savedData.generalTimes);
              setGeneralTimes(savedData.generalTimes);
            } else {
              console.log(`[AvailabilityStep:${role}] No generalTimes to restore`);
            }

            // Restore detailed schedule
            if (savedData.availabilityPeriods) {
              console.log(`[AvailabilityStep:${role}] Setting availabilityPeriods:`, savedData.availabilityPeriods);
              setAvailabilityPeriods(savedData.availabilityPeriods);
            }
            if (savedData.unavailabilityPeriods) {
              console.log(`[AvailabilityStep:${role}] Setting unavailabilityPeriods:`, savedData.unavailabilityPeriods);
              setUnavailabilityPeriods(savedData.unavailabilityPeriods);
            }
          } else {
            console.log(`[AvailabilityStep:${role}] ⚠️ No saved availability data found`);
          }

          console.log(`[AvailabilityStep:${role}] Setting isRestored to true`);
          setIsRestored(true);
        })
        .catch(error => {
          console.error(`[AvailabilityStep:${role}] ❌ Error loading saved progress:`, error);
          setIsRestored(true);
        });
    } else {
      if (isRestored) {
        console.log(`[AvailabilityStep:${role}] Skipping restoration - already restored`);
      }
      if (!user?.id) {
        console.log(`[AvailabilityStep:${role}] Skipping restoration - no user ID`);
      }
    }
  ...
  const detectConflicts = useCallback((periods: AvailabilityPeriod[], unavailPeriods: UnavailabilityPeriod[]): Map<string, string[]> => {
  ...
  }, [availabilityPeriods, unavailabilityPeriods, detectConflicts]);
  ...
  const handleNext = () => {
    console.log(`[AvailabilityStep:${role}] handleNext called`);
    console.log(`[AvailabilityStep:${role}] Form data:`, availabilityData);
    ...
    console.log(`[AvailabilityStep:${role}] onNext called successfully`);
  };
      ...
      <div className={styles.stepHeader}>
        <div>
          ...
          <p className={styles.stepSubtitle}>
            {roleDisplayName} Onboarding • Let students know when you&apos;re available
          </p>
        </div>
      </div>
