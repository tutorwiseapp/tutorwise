...
class JiraClient {
   ...
   */
  async pullAIReadyTasks(jql: string | null = null): Promise<CasTask[]> {
    ...
    try {
      ...
      return response.data.issues.map((issue: JiraIssue) => this.formatTask(issue));
    } catch (error: any) {
   ...
   */
  formatTask(issue: JiraIssue): CasTask {
   ...
   */
  extractAcceptanceCriteria(description: string): { text: string; completed: boolean }[] {
    const criteria: { text: string; completed: boolean }[] = [];
  ...
  async updateTaskStatus(taskId: string, status: string): Promise<boolean> {
    try {
      ...
      const transition = transitionsResp.data.transitions.find((t: any) =>
   ...
   */
  generateAIComment(task: CasTask, pr: any): string {
...
**Changes Made:**
${pr.changes.map((c: string) => `- ${c}`).join('\n')}
...
Generated by CAS Autonomous Agent v1.0.0`;
   ...
   * Assign task to user
   */
  async assignTask(taskId: string, userId: string | null = null): Promise<boolean> {
    try {
