 ...
 */
async function applyAdaptations(packageDir, rulesFile, targetPlatform) {
 ...
 */
async function applyRule(packageDir, rule, targetPlatform) {
  switch (rule.type) {
    case 'string_replace':
 ...
 */
async function applyStringReplace(packageDir, rule) {
 ...
 */
async function applyRegexReplace(packageDir, rule) {
  const filePatterns = Array.isArray(rule.files) ? rule.files : [rule.files];
  let changeCount = 0;
 ...
 */
async function applyImportRewrite(packageDir, rule) {
 ...
 */
async function applyTypeMapping(packageDir, rule) {
 ...
 */
async function applyFileExclusion(packageDir, rule) {
 ...
 */
async function applyAddImport(packageDir, rule) {
  const filePatterns = Array.isArray(rule.files) ? rule.files : [rule.files];
  let changeCount = 0;
 ...
 */
async function applyRemoveCodeBlock(packageDir, rule) {
  const filePatterns = Array.isArray(rule.files) ? rule.files : [rule.files];
  let changeCount = 0;
